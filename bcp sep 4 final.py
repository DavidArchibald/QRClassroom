...
unused_br_pass = [') bathroom pass #1',
                  ') bathroom pass #2',
                  ') bathroom pass #3'
                 ]
used_br_pass = []
...
def Main():
    ...
    while qr_ID_input[0] == '&' and qr_ID_input in str(premade_qr_ID):
        ...
        if str(qr_ID_input) not in str(my_dict):
            ...
            print 'Welcome ' + name \
                + ', Place your name label on the back of your Science ID'
            ...
            f = open("c:\\barcodeprogram\qrscanID\\available_qr_ID.txt"
                     , 'w+')
            f.write(str(available_qr_ID))
            f.close()

            f = open("c:\\barcodeprogram\qrscanID\\premade_qr_ID.txt",
                     'w+')
            f.write(str(premade_qr_ID))
            f.close()

            f = open("c:\\barcodeprogram\qrscanID\\used_qr_codes.txt",
                     'w+')
            f.write(str(usedID))
            f.close()
...
def action_scan_input():
    ...
    if action[0] == '(':
        ...
        with open("c:\\barcodeprogram\qrscanID\\my_dict.txt", 'r') as \
            inf:
            my_dict = eval(inf.read())
        ...
        if borrow in useditem and qrid in usedID:
            ...
            f = open("c:\\barcodeprogram\qrscanID\\check_in_out.txt",
                     'a+')
            f.write(name + ' ,' + ' returned ' + borrow + ' on '
                    + localtime + '\n')
            f.close()
            ...
            print name + 'checked out' + str(borrow) + ' on ' \
                + localtime + '\n'
    ...
    if action[0] == ')':
        ...
        with open("c:\\barcodeprogram\qrscanID\\my_dict.txt", 'r') as \
            ...
            if str(br_pass) in str(unused_br_pass):
                ...
                f = \
                    open("c:\\barcodeprogram\qrscanID\\bathroom_in_out.txt"
                         , 'a+')
                f.write(str(name) + 'left on ' + str(localtime)
                        + ' with' + str(used_br_pass) + '\n')
                f.close()

                print str(name) + 'left on ' + str(localtime) + ' with' \
                    + str(used_br_pass) + '\n'
                ...
                f = \
                    open("c:\\barcodeprogram\qrscanID\\bathroom_in_out.txt"
                         , 'a+')
                f.write(str(name) + 'returned on ' + str(localtime)
                        + ' with' + str(used_br_pass) + '\n')
                f.close()

                print str(name) + 'returned on ' + str(localtime) \
                    + ' with' + str(used_br_pass) + '\n'
...
# This admin function is to re-activate used tickets
    ...
    if action[0] == '!':
        ...
        while action[0] == '!':
            renew = \
                input('reactivate used tickets now.  When done scan Purple Admin QR code'
                      )
